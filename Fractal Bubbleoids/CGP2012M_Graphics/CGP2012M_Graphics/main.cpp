...
int main(int argc, char *argv[]) {
	...
	//create player
	player = new Triangle_T(centreX, centreY);
	lives = 3;
	...
	glUseProgram(player->shaderProgram1);
	translate = glm::translate(translate, glm::vec3(2.0f, 1.5f, 0.0f));
	modelLocation = glGetUniformLocation(player->shaderProgram1, "uModel");
	...
	while (windowOpen)
	{
		...
		projectionLocation = glGetUniformLocation(player->shaderProgram1, "uProjection");
		glUniformMatrix4fv(projectionLocation, 1, GL_FALSE, glm::value_ptr(projectionMatrix));
		modelLocation = glGetUniformLocation(player->shaderProgram1, "uModel");
		glUniformMatrix4fv(modelLocation, 1, GL_FALSE, glm::value_ptr(translate*rotate*scale));
		viewLocation = glGetUniformLocation(player->shaderProgram1, "uView");
		glUniformMatrix4fv(viewLocation, 1, GL_FALSE, glm::value_ptr(viewMatrix));
